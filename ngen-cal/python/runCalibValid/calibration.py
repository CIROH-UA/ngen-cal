...
def main(general: General, model_conf):
    ...
    if general.strategy.algorithm == Algorithm.dds:
        ...
        if agent.model.strategy != "uniform":
            logging.error("Can only use PSO with the uniform model strategy")
            return
        if general.restart:
            logging.warning("Restart not supported for PSO search, starting at 0")
        func = pso_search
    elif general.strategy.algorithm == Algorithm.gwo:
        if agent.model.strategy != "uniform":
            logging.error("Can only use GWO with the uniform model strategy")
            return
        if general.restart:
            start_iteration = agent.restart()
        func = gwo_search

    logging.info("Starting Iteration: {}".format(start_iteration))
    logging.info("Starting calibration loop")
