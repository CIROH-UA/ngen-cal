...
class NgenBase(ModelExec):
    ...
    def __init__(self, **kwargs):
        ...
        # there are no coordinates, just a time dimension and a catchment dimension
        # the time dimension is an index to a 2d array called Time that is catchment-id x timesteps in unix time
        data = xr.open_dataset(self.ngen_realization.global_config.forcing.path)

        times = data.Time[0].values
        data = data.APCP_surface
        times = [dt.datetime.utcfromtimestamp(t) for t in times]

        precip = data.sum(dim="catchment-id")
        # replace the "time" index dimension with actual datetime objects
        precip = precip.assign_coords(time=times)
        precip = precip.rename({"time": "Time"})
        precip = precip.rename("RAINRATE")
        self._precip = precip.to_dataframe().reset_index()
    ...
    def __init__(self, **kwargs):
        ...
        for (
            ...
            try:
                ...
                KeyError
            ):  # This probaly isn't strictly required since we built these from the index
                continue
            try:
                nexus_data = self._nexus_hydro_fabric.loc[fabric["toid"]]
            except KeyError:
        ...
        self._catchments.append(
            CalibrationSet(
                catchments, eval_nexus[0], self.routing_output, start_t, end_t, self.eval_params
            )
        )
...
class NgenUniform(NgenBase):
    ...
    def __init__(self, **kwargs):
        ...
        for id, toid in self._catchment_hydro_fabric["toid"].items():
            ...
            # nexus = Nexus(nexus_data.name, location, (), id)
            nexus = Nexus(nexus_data.name, location)
        ...
        nexus_id = self._catchment_hydro_fabric.loc[self._x_walk.index[0].replace("cat", "wb")][
            "toid"
        ]
        self._wb_lst = [
            x.split("-")[1]
