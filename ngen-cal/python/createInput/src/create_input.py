...
def main():
    ...
    cfe_overland_routing = config.get(section, 'cfe_overland_routing')
    ...
    if not os.path.exists(cat_file):
        ...
        shutil.copy(gpkg_file, cat_file)
    ...
    for catID in catids:
        ...
        if not os.path.exists(os.path.join(forcing_path, os.path.basename(ffile))):
            ...
            shutil.copy(ffile, os.path.join(forcing_path, os.path.basename(ffile)))
    ...
    if model in ['cfe_noah', 'cfe_noah_sft', 'cfe_xaj_noah', 'cfe_xaj_noah_sft']:
        gfun.create_cfe_input(cfe_overland_routing, catids, attr_file, cfe_input_dir)
